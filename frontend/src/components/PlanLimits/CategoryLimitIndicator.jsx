...
export default function CategoryLimitIndicator({ limits }) {
  if (!limits || limits.plan !== 'FREE') return null;

  const { categoryCount, categoryLimit } = limits;
  const percent = Math.min((categoryCount / categoryLimit) * 100, 100);
  const isAtLimit = categoryCount >= categoryLimit;
